...
def window_partition(x, window_size):
    ...
    """
    B, H, W, C = x.shape  # 1,64,64,1  64*64=4096
    # print(B,H // window_size, window_size,W//window_size,window_size,C)
    x = x.view(B, H // window_size, window_size, W // window_size, window_size, C)  # 63*63?
...
class RSTB(nn.Module):
    ...
    def __init__(self, dim, input_resolution, depth, num_heads, window_size,
                 mlp_ratio=4., qkv_bias=True, qk_scale=None, drop=0., attn_drop=0.,
                 drop_path=0., norm_layer=nn.LayerNorm, downsample=None, use_checkpoint=False,
                 img_size=64, patch_size=4, resi_connection='conv1'):
...
class RED_CNN_transformer(nn.Module):
    def __init__(self, img_size=64, patch_size=1,in_chans=1,
                 embed_dim=96, depths=[2,2], num_heads=[6,6],
                 window_size=8, mlp_ratio=4., qkv_bias=True, qk_scale=None,
                 drop_rate=0., attn_drop_rate=0.,drop_path_rate=0.1,
                 norm_layer=nn.LayerNorm, ape=False, patch_norm=True, upscale=1, img_range=1,resi_connection='conv1'):
        super(RED_CNN_transformer, self).__init__()
        num_in_ch = in_chans
        num_out_ch = in_chans
        num_feat = 64
        ...
        self.texture = nn.Conv2d(1,96,kernel_size=3,stride=1,padding=1)
        self.relu = nn.ReLU()
        ...
        # build Residual Swin Transformer Blocks    (residual?)
        ...
        # build the last conv layer in deep extraction
        if resi_connection == 'conv1':
            self.conv_after_body = nn.Conv2d(embed_dim, embed_dim, 3, 1, 1)
        ################################### 3. deconv
        self.conv_t = nn.ConvTranspose2d(embed_dim,embed_dim, 3, 1, 1)
        self.conv_t_last = nn.ConvTranspose2d(embed_dim, 1, 3, 1, 1)
    # init
    ...
    # define the forward of deep extraction
    ...
    def forward(self, x):
        # encoder
        residual_1 = x.clone()
        out = self.relu(self.texture(x))#1,64
        # out = self.conv_after_body(self.forward_features(out)) + out # 64,64
        out = self.conv_after_body(self.forward_features(out))  # 64,64
        residual_2 = out.clone()
        out = self.conv_after_body(self.forward_features(out))
        out = self.conv_after_body(self.forward_features(out))
